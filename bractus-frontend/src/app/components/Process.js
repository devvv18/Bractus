...
// Single continuous path: entry line and outer contour of the logo (no inner loop cutout stroke to avoid double outline)
// Starts off-screen, curves through "w" (390.78, 218.40), traces outer loop, and returns to "w".
...
export function HowWeWork() {
  ...
  // 1. Single Path Stroke Length (Head of the snake):
  // - 0.04 to 0.20 scroll: draws the entry sweep line to "w" (0.0 to 0.21 path length)
  // - 0.20 to 0.45 scroll: traces the logo shape (0.21 to 1.0 path length)
  const rawPathDraw = useTransform(scrollYProgress, [0.04, 0.20, 0.45], [0, 0.21, 1], { clamp: true })
  const pathDraw = useSpring(rawPathDraw, { stiffness: 60, damping: 28 })

  // 2. Single Path Stroke Offset (Tail of the snake):
  // - 0.04 to 0.20 scroll: tail stays at the start point (0.0 offset)
  // - 0.20 to 0.45 scroll: tail slides to "w" (0.0 to 0.21 offset), erasing the entry line behind it
  const rawPathOffset = useTransform(scrollYProgress, [0.04, 0.20, 0.45], [0, 0, 0.21], { clamp: true })
  const pathOffset = useSpring(rawPathOffset, { stiffness: 60, damping: 28 })

  // 3. Solid Fill: is revealed by the mask. We map its opacity to make it fully solid as the sweep starts
  const fillOpacity = useTransform(pathDraw, [0.20, 0.25], [0, 0.95], { clamp: true })

  // 4. Outline Stroke Opacity: fades out outline stroke snappily once the tail catches up to "w" (pathOffset reaches 0.21)
  const outlineOpacity = useTransform(pathOffset, [0.18, 0.21], [1, 0], { clamp: true })
          ...
          <div style={{
            ...
            >
              <defs>
                ...
                {/* Logo reveal mask using COMBINED_PATH as a thick guide stroke */}
                <mask id="hw-logo-mask">
                  <motion.path
                    d={COMBINED_PATH}
                    fill="none"
                    stroke="white"
              ...
              {/* 1. Single Continuous Drawing Path (draws to 'w', traces outer contour, then erases tail) */}
              <motion.path
                d={COMBINED_PATH}
                fill="none"
              ...
              {/* 3. Solid Filled Bractus Brand Logo (revealed as a thick layer directly all at once via mask) */}
              <motion.path
                d={BRAND_LOGO_PATH}
                fill="url(#hw-brand-grad)"
                mask="url(#hw-logo-mask)"
                style={{
                  opacity: fillOpacity,
                }}
              />
